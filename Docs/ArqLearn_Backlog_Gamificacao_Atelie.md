...
1. **Agora, antes de qualquer coisa do backlog faseado:** consertar o gap de cosméticos (achado da comparação, não uma spec nova — expor em UI o que já está gravado em `purchases`, sem exigir feature flag nem sistema novo). É a menor unidade de trabalho de todo este documento.
2. **Curto prazo (Fase 1, e só uma peça de cada vez, RG-03):** dentro da Fundação, a peça de menor esforço isolado é **1.1 Eventos e telemetria** — puramente aditiva, sem UI nova, e é o bloqueio estrutural de tudo o mais (RT-01). Depois dela, **1.3 XP (delta do combo)** é pequena porque só troca uma fórmula já existente por outra parecida.

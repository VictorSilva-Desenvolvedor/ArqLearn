...
// accessToken opcional pro mesmo motivo de getGamificationProfile (Server Components).

export async function getVipStatus(accessToken?: string): Promise<VipStatus> {
  if (isResourceReal("gamification")) {
    return apiFetch<VipStatus>("/v1/vip/status", undefined, accessToken);
  }

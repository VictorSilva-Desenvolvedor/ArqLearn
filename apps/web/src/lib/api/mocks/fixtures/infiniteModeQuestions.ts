...
// Banco "difícil" por tema — Modo Infinito usa perguntas de dificuldade elevada por design
// (ver Docs/stitch_app_visual_identity/modo_infinito_desafio_estrutural). Um banco por tema do
// catálogo em themes.ts, pra a troca de tema no TopAppBar refletir de verdade no conteúdo do
// Modo Infinito, não só no rótulo da tela.
...
// Reaproveita 12 perguntas "hard" já geradas e aprovadas (Gemini/Groq/Claude, ver
// services/monolith/seeds/002_maquetes_licoes_perguntas.js, 003_maquetes_perguntas_claude.js e
// 004_maquetes_perguntas_claude_batch2.js) pras 4 lições reais de Maquetes no MongoDB — mesmo
// texto, só reformatado pro schema do mock de Modo Infinito (options viram {id,label}, sem
// repetir a chamada de IA pra um conteúdo que já existe e já foi validado). O banco real de
// Maquetes cresceu bem além dessas 12 (163 perguntas nas 4 dificuldades, incl. o novo nível
// "impossible") — o Modo Infinito usa só uma amostra "hard", por design (ver comentário acima).

...
// Espelha apps/web/.../resources/gems.ts.

export async function getGemTransactions(cursor?: string, limit = 20): Promise<GemTransactionsPage> {
  if (isResourceReal("gamification")) {
    const query = new URLSearchParams();
    if (cursor) query.set("cursor", cursor);
    if (limit) query.set("limit", String(limit));
    const qs = query.toString();
    return apiFetch<GemTransactionsPage>(`/v1/gamification/gem-transactions${qs ? `?${qs}` : ""}`);
  }
...
export async function getGemPackages(): Promise<GemPackage[]> {
  if (isResourceReal("gamification")) {
    const { data } = await apiFetch<{ data: GemPackage[] }>("/v1/gamification/gem-packages");
    return data;
  }
...
export async function getActiveGemBet(): Promise<GemBet | null> {
  if (isResourceReal("gamification")) {
    return apiFetch<GemBet | null>("/v1/gamification/bets/active");
  }
